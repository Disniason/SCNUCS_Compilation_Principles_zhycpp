    ...
    class grammar_dfa_graph{
        ...
        class grammar_dfa_node{
            ...
            bool actionConflicting() const{
                ...
                for(const auto & grammar : grammars){
                    if (grammar.action != firstAct) return true;
                }
                return false;
            }

            bool reductionConflicting(const zhy::grammar_first_follow_set & gffs) const{
                std::unordered_set <std::string> reductionStartSet;
                for(const auto & grammar : grammars){
                    if (grammar.action){
                        reductionStartSet.emplace(grammar.getStart());
                        //std::cout<<grammar.getStart()<<std::endl;
                    }
                }
                if (reductionStartSet.empty()) return false;
                std::unordered_set <std::string> followSet = gffs.non_terminal_follow_set.at(*reductionStartSet.begin());
                for(auto it = std::next(reductionStartSet.begin());it != reductionStartSet.end();it++){
                    const auto & tempFollowSet = gffs.non_terminal_follow_set.at(*it);
                    for(const auto & element : tempFollowSet){
                        if (followSet.find(element) != followSet.end()) return true;
                        else followSet.emplace(element);
                    }
                }
                return false;
            }

            bool SLRConflicting(const zhy::grammar_first_follow_set & gffs,bool & flag) const{
                if (actionConflicting()){
                    flag = 0;
                    return true;
                }
                else if (reductionConflicting(gffs)){
                    flag = 1;
                    return true;
                }
                else return false;
        ...
        */
        ...
        bool isSLR1(const zhy::grammar_first_follow_set & gffs,std::ostream & description = std::cout) const{
            for(const auto & node : id_node_map){
                bool flag;
                if (node.second.SLRConflicting(gffs,flag)){
